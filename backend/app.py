...
app = FastAPI(title="Summarizer.ai", description="AI research paper summarizer and Insight extractor")
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    user = get_user_by_email(form_data.username) or get_user_by_username(form_data.username)
...
# ── Feature 2: History ──
@app.get("/history")
async def get_user_history(current_user: dict = Depends(get_current_user_optional)):
    if current_user:
...
@app.post("/compare")
async def compare_papers(data: CompareInput, current_user: dict = Depends(get_current_user_optional)):
    result = groq_chat(
...
async def export_compare(data: CompareExportInput, current_user: dict = Depends(get_current_user_optional)):
    pdf_doc = fitz.open()
