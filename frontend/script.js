...
// Temporarily disabled for testing: if (!token) window.location.href = 'login.html';

const userDisp = document.getElementById('usernameDisplay');
if (userDisp) userDisp.textContent = localStorage.getItem('username') || '';
...
analyzeBtn.addEventListener('click', async () => {
    ...
    if (document.getElementById('progressWrap')) document.getElementById('progressWrap').style.display = 'block';
    if (results) results.style.display = 'none';
    const absWarn = document.getElementById('abstractWarning');
...
async function summarizeText(text) {
    ...
    if (res.status === 401) { logout(); return; }
    if (!res.ok) { const e = await res.json(); throw new Error(e.detail || 'Processing failed'); }
    return res.json();
}
...
function showResults(data) {
    ...
    if (data.is_abstract_only) {
        const absWarn = document.getElementById('abstractWarning');
        if (absWarn) absWarn.style.display = 'block';
    }
...
// Q&A and Chat logic handled via qaInput/qaMessages and chatWidget
...
const cp1 = document.getElementById('comparePdf1');
const cp2 = document.getElementById('comparePdf2');
const cz1 = cp1?.closest('.drop-zone');
const cz2 = cp2?.closest('.drop-zone');
...
if (cz1) {
    ...
    cz1.addEventListener('drop', async (e) => {
        e.preventDefault(); cz1.classList.remove('over');
        const file = e.dataTransfer.files[0];
        if (file?.type === 'application/pdf') await handleCompareUpload(file, 1);
    });
}
if (cz2) {
    ...
    cz2.addEventListener('drop', async (e) => {
        e.preventDefault(); cz2.classList.remove('over');
        const file = e.dataTransfer.files[0];
        if (file?.type === 'application/pdf') await handleCompareUpload(file, 2);
    });
}

cp1?.addEventListener('change', async (e) => { if (e.target.files[0]) await handleCompareUpload(e.target.files[0], 1); });
cp2?.addEventListener('change', async (e) => { if (e.target.files[0]) await handleCompareUpload(e.target.files[0], 2); });

async function handleCompareUpload(file, num) {
    const dropText = document.getElementById(`compareDropText${num}`);
    const textArea = document.getElementById(`compareText${num}`);
    const titleInput = document.getElementById(`title${num}`);
    
    if (dropText) dropText.textContent = '⏳ Extracting...';
    const text = await extractPdfText(file);
    if (text) {
        if (textArea) textArea.value = text;
        if (dropText) dropText.textContent = '✓ ' + file.name;
        if (titleInput && !titleInput.value) titleInput.value = file.name.replace('.pdf','');
    } else {
        if (dropText) dropText.textContent = 'Upload PDF';
    }
}
...
// Explorer logic handled if needed
...
if (compareBtn) {
    ...
    try {
        ...
        resultDiv.innerHTML = `<h3 style="margin-bottom:1rem;color:var(--purple)">⚖️ ${title1} vs ${title2}</h3><p style="white-space:pre-wrap;color:var(--sub);line-height:1.8">${data.comparison}</p>`;
        resultDiv.scrollIntoView({ behavior: 'smooth' });

        // Show download buttons
        const dlRow = document.getElementById('compareDownloadRow');
...
function confirmDialog(msg) {
    return new Promise(resolve => {
        ...
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(3,7,18,0.85);backdrop-filter:blur(8px);z-index:9000;display:flex;align-items:center;justify-content:center';
        overlay.innerHTML = `<div style="background:#0f172a;border:1px solid rgba(45,212,191,0.2);border-radius:24px;padding:2.5rem;max-width:400px;width:90%;text-align:center;box-shadow:0 25px 50px rgba(0,0,0,0.5)">
            <p style="margin-bottom:2rem;font-size:1.1rem;color:#f8fafc;font-weight:500">${msg}</p>
            <div style="display:flex;gap:1rem;justify-content:center">
                <button id="confirmYes" style="padding:0.8rem 1.8rem;background:linear-gradient(135deg,#2dd4bf,#38bdf8);color:#030712;border:none;border-radius:12px;cursor:pointer;font-weight:800;text-transform:uppercase;letter-spacing:0.05em">Yes</button>
                <button id="confirmNo" style="padding:0.8rem 1.8rem;background:transparent;border:1px solid rgba(255,255,255,0.1);color:#94a3b8;border-radius:12px;cursor:pointer;font-weight:700">Cancel</button>
            </div></div>`;
